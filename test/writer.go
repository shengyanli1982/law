...
func NewWriteAsyncer(w io.Writer, conf *Config) *WriteAsyncer {
	...
	wa.bufferPool = buf.NewExtraBufferPool(wa.config.buffsize)
...
func (wa *WriteAsyncer) poller() {
	...
	exec := func(ln *list.Node) {
		...
		// wa.config.callback.OnPopQueue(bytes, now-eb.UpdateAt()) // 回调函数 (Callback function)
...
func (wa *WriteAsyncer) buffWriter(p []byte) (int, error) {
	wa.config.callback.OnWrite(p)                                                   // 回调函数 (Callback function)
...
func (wa *WriteAsyncer) Write(p []byte) (int, error) {
	...
	wa.config.callback.OnPushQueue(p) // 回调函数 (Callback function)
